...
exports.postSignup = async (req, res) => {
  ...
  return res.status(200).json({
    message:
      "Thank you for signing up for our newsletter! Please complete the process by confirming the subscription in your email inbox.",
  });
};
...
exports.getConfirm = async (req, res) => {
  try {
    ...
    return res
      .status(400)
      .json({ message: "Subscription was unsuccessful. Please try again" });
  }
};
...
async function sendNewsletterToList(req, htmlNewsletter, listID) {
  ...
  for (const subscriber of response[1].result) {
    ...
    const unsubscribeURL =
      req.protocol + "://" + req.headers.host + "/delete/?" + params;
    const msg = {
      to: subscriber.email,
...
async function getContactByEmail(email) {
  ...
  if (response[1].result[email]) return response[1].result[email].contact;
    ...
    return res
      .status(400)
      .send({ success: false, message: "An error occured", data: null });
}
