...
const { GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, SERVER_ROOT_URI, LINKEDIN_URL_ENCODED, LINKEDIN_CALLBACK_URL, LINKEDIN_CLIENT_ID, LINKEDIN_SECRET_ID, FB_CLIENT_ID, FB_CLIENT_SERECT } = environment;
...
const getLinkedinUrl = (req, res) => {
	return res.status(200).json(response ({ message: "LinkedIn URL", success: true, data: `https://www.linkedin.com/oauth/v2/authorization?response_type=code&scope=r_liteprofile%20r_emailaddress&client_id=${LINKEDIN_CLIENT_ID}&redirect_uri=${LINKEDIN_URL_ENCODED}` }));
}
...
/*
	...
	* that can be used to fetch the user's profile
*/
const getLinkedinAccessToken = async ({ code, clientId, clientSecret, redirectUri }) => {
...
const linkedinAccessToken = async(req, res) => {
	...
	try {
		...
		if (existing_user) {
			return res.status(200).json(response({ success: true,	message: "User login successfully",	data: authResponse(existing_user)	}))
		}
...
module.exports = { googleAuthURL, getTokens, getLinkedinUrl, linkedinAccessToken };
